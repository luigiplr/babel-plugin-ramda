...
exports.default = function (_ref) {
  ...
  function importMethod(methodName, file) {
    if (!selectedMethods[methodName]) {
      ...
      selectedMethods[methodName] = (0, _helperModuleImports.addNamed)(file.path, 'default', path);
    }
  ...
  return {
    visitor: {
      ...
      ExportNamedDeclaration: function ExportNamedDeclaration(path) {
        ...
        if (node.source && node.source.value === 'ramda') {
          var specifiers = node.specifiers.map(function (spec) {
            var importIdentifier = importMethod(spec.exported.name, hub.file);
      ...
      CallExpression: function CallExpression(path) {
        ...
        if (node.arguments) {
          node.arguments = node.arguments.map(function (arg) {
            ...
            return matchesRamdaMethod(path, name) ? importMethod(specified[name], hub.file) : arg;
          });
        }
      },
      MemberExpression: function MemberExpression(path) {
        ...
        // R.foo() -> foo()
        var newNode = importMethod(node.property.name, path.hub.file);
      ...
      Identifier: function Identifier(path) {
        ...
        if (matchesRamdaMethod(path, name) && !isSpecialTypes(t, parent)) {
          var newNode = importMethod(specified[name], hub.file);
          path.replaceWith({ type: newNode.type, name: newNode.name });
